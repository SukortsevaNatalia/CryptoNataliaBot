...
class CryptoConverter:
    ...
    def get_price(base: str, quote: str, amount: str):
        if quote == base:
            raise APIException(f'Вы ввели две одинаковые валюты. Невозможно перевести одинаковые валюты {base}.')

        try:
            quote_ticker = keys[quote]
        except KeyError:
            raise APIException(f'Нет такой валюты как {quote}')

        try:
            base_ticker = keys[base]
        except KeyError:
            raise APIException(f'Нет такой валюты как {base}')
